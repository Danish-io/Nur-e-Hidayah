...
import { SurahOverlay } from "@/components/quran/SurahOverlay";
...
const SALAH_GUIDE = [
    ...
    },
    {
        ...
        icon: "📖",
        steps: [
            { text: "Surah Al-Qadr (97) — Recite at least 7 times. Its reward is immense on this night.", surahId: 97 },
            { text: "Surah Al-Ikhlas (112) — Recite 3 times in every Rakat (equals reciting the full Quran).", surahId: 112 },
            { text: "Surah Al-Falaq (113) & Surah An-Nas (114) — Read for protection from evil.", surahId: 113, surahId2: 114 },
            { text: "Surah Al-Mulk (67) — Protects from the punishment of the grave.", surahId: 67 },
            { text: "Surah Al-Baqarah (2:255) — Ayat-ul-Kursi after every prayer.", surahId: 2, verseId: 255 },
        ],
    ...
    },
    {
        ...
        icon: "📕",
        steps: [
            { text: "After Isha: Recite Juz 28 (Surah Al-Mujadeela to Al-Tahrim).", juzId: 28 },
            { text: "After Tarawih: Recite Juz 29 (Al-Mulk to Al-Mursalat).", juzId: 29 },
            { text: "Tahajjud (midnight): Recite Juz 30 (Al-Naba to An-Nas) — the final Juz.", juzId: 30 },
            { text: "Pre-Sehri (before Fajr): Complete with Surah Al-Qadr and Al-Baqarah's last 2 verses.", surahId: 97, surahId2: 2, verseId2: 285 },
            { text: "Listen to Quran recitation if you become tired — the reward continues." },
        ],
...
export default function ShabEQadrPage() {
    ...
    const [readingJuzId, setReadingJuzId] = useState<number | null>(null);

    const handleReadSurah = (surahId?: number | null, verseId?: number | null, juzId?: number | null) => {
        setReadingSurahId(surahId || null);
        setReadingVerseId(verseId || null);
        setReadingJuzId(juzId || null);
    };
                ...
                <AnimatePresence mode="wait">
                    ...
                    {activeTab === "namaz" && (
                        <motion.div key="namaz" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }} className="space-y-12">
                            {SHAB_E_QADR_NAMAZ.map((namaz) => <NamazCard key={namaz.id} namaz={namaz} onReadSurah={handleReadSurah} />)}
                        </motion.div>
                    )}
                    ...
                    {activeTab === "salah" && (
                        <motion.div key="salah" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }} className="space-y-12">
                            {SALAH_GUIDE.map((item) => <SalahCard key={item.id} item={item} onReadSurah={handleReadSurah} />)}
                        </motion.div>
                    )}
                </AnimatePresence>

                {/* Surah Reading Overlay */}
                <SurahOverlay 
                    surahId={readingSurahId} 
                    juzId={readingJuzId}
                    initialVerseId={readingVerseId}
                    isOpen={!!readingSurahId || !!readingJuzId} 
                    onClose={() => {
                        setReadingSurahId(null);
                        setReadingVerseId(null);
                        setReadingJuzId(null);
                    }} 
                />
            </div>
        </div>
...
// ─── Nawafil Card ────────────────────────────────────────────────────
function NamazCard({ namaz, onReadSurah }: { namaz: ShabEQadrNamaz, onReadSurah?: (surahId: number | null, verseId?: number | null, juzId?: number | null) => void }) {
                            ...
                            <div className="pt-2 pb-4 max-w-2xl mx-auto border-t border-gray-100 dark:border-zinc-800 mt-2">
                                ...
                                <ul className="space-y-4">
                                    {namaz.method.map((step, idx) => {
                                        ...
                                        const text = isObject ? (step as any).text : step;
                                        const surahId = isObject ? (step as any).surahId : null;
                                        const surahId2 = isObject ? (step as any).surahId2 : null;
                                        ...
                                        return (
                                            <li key={idx} className="flex flex-col gap-3">
                                                ...
                                                {isObject && (surahId || surahId2) && (
                                                    <div className="ml-9 flex gap-2">
                                                        {surahId && (
                                                            <Button 
                                                                variant="outline" 
                                                                size="sm" 
                                                                className="h-8 px-4 rounded-full text-[11px] font-bold border-indigo-100 dark:border-indigo-900/50 hover:bg-emerald-600 hover:text-white dark:hover:bg-emerald-600 transition-all gap-1.5"
                                                                onClick={() => onReadSurah?.(surahId, (step as any).verseId || null)}
                                                            >
                                                                <BookOpen className="w-3.5 h-3.5" /> 
                                                                { (step as any).verseId ? "Read Ayat" : "Read Surah" }
                                                            </Button>
                                                        )}
                                                        {surahId2 && (
                                                            <Button 
                                                                ...
                                                                size="sm" 
                                                                className="h-8 px-4 rounded-full text-[11px] font-bold border-indigo-100 dark:border-indigo-900/50 hover:bg-emerald-600 hover:text-white dark:hover:bg-emerald-600 transition-all gap-1.5"
                                                                onClick={() => onReadSurah?.(surahId2, (step as any).verseId2 || null)}
                                                            >
                                                                <BookOpen className="w-3.5 h-3.5" /> 
                                                                { (step as any).verseId2 ? "Read Ayat" : "Read Surah" }
                                                            </Button>
                                                        )}
                                                    </div>
                                                )}
...
// ─── Salah Guide Card ─────────────────────────────────────────────────
function SalahCard({ item, onReadSurah }: { item: typeof SALAH_GUIDE[0], onReadSurah?: (surahId?: number | null, verseId?: number | null, juzId?: number | null) => void }) {
    const [showDetails, setShowDetails] = useState(false);
            ...
            {item.steps.length > 0 && (
                <div className="flex flex-col items-center">
                    ...
                    <AnimatePresence>
                        {showDetails && (
                            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden w-full">
                                <ul className="space-y-4 pb-2 pt-4 border-t border-gray-100 dark:border-zinc-800">
                                    {item.steps.map((step, idx) => {
                                        const isObject = typeof step === 'object';
                                        const text = isObject ? (step as any).text : step;
                                        const surahId = isObject ? (step as any).surahId : null;
                                        const surahId2 = isObject ? (step as any).surahId2 : null;
                                        const juzId = isObject ? (step as any).juzId : null;
                                                ...
                                                <div className="flex items-start gap-4">
                                                    ...
                                                    <div className="flex-1 flex flex-col md:flex-row md:items-center justify-between gap-3">
                                                        ...
                                                        <div className="flex gap-2">
                                                            {(surahId || (isObject && (step as any).verseId)) && (
                                                                <Button 
                                                                    variant="outline" 
                                                                    size="sm" 
                                                                    className="h-8 px-4 rounded-full text-[11px] font-bold border-indigo-100 dark:border-indigo-900/50 hover:bg-indigo-600 hover:text-white dark:hover:bg-indigo-600 transition-all gap-1.5"
                                                                    onClick={() => onReadSurah?.(surahId || (step as any).surahId || null, (step as any).verseId || null)}
                                                                >
                                                                    <BookOpen className="w-3.5 h-3.5" /> 
                                                                    { (isObject && (step as any).verseId) ? "Read Ayat" : "Read Surah" }
                                                                </Button>
                                                            )}
                                                            {juzId && (
                                                                <Button 
                                                                    ...
                                                                    size="sm" 
                                                                    className="h-8 px-4 rounded-full text-[11px] font-bold border-indigo-100 dark:border-indigo-900/50 hover:bg-indigo-600 hover:text-white dark:hover:bg-indigo-600 transition-all gap-1.5"
                                                                    onClick={() => onReadSurah?.(null, null, juzId)}
                                                                >
                                                                    <BookOpen className="w-3.5 h-3.5" /> Read Juz
                                                                </Button>
                                                            )}
                                                            {surahId2 && (
                                                                <Button 
                                                                ...
                                                                >
                                                                    <BookOpen className="w-3.5 h-3.5" /> 
                                                                    { (isObject && (step as any).verseId2) ? "Read Baqarah" : "Read Nas" }
                                                                </Button>
                                                            )}
                                                        </div>
                                                    </div>
