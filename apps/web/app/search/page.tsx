...
export default function SearchPage() {
    ...
    const [messages, setMessages] = useState<{ role: 'user' | 'ai', content: string }[]>([
        { role: 'ai', content: "Assalamu Alaikum! I am Nur-e-Hidayah. Ask me any question about the Quran, and I will find relevant verses and tafseer for you." }
    ]);
    ...
    const handleSearch = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setMessages(prev => [...prev, { role: 'ai', content: data.answer || "Sorry, I couldn't find an answer." }]);
        } catch (err) {
                    ...
                    {messages.map((msg, idx) => (
                        <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[80%] p-4 rounded-2xl ${msg.role === 'user'
                                ...
                                <p className="text-sm md:text-base leading-relaxed whitespace-pre-wrap select-text">{msg.content}</p>
                            </div>
                        </div>
                    ))}
                ...
                <form onSubmit={handleSearch} className="fixed bottom-0 left-0 right-0 p-4 bg-white/80 dark:bg-black/80 backdrop-blur border-t border-gray-200 dark:border-zinc-800">
                    <div className="container mx-auto max-w-3xl relative flex items-center">
                        <input
                            ...
                            placeholder={isListening ? "Listening..." : "Ask a question (e.g., 'What does the Quran say about patience?')"}
